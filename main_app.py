...
if tabs == 'Introduction':
    ...
    st.markdown("1a) How do consumer spending and long-term interest rates affect the stock market (return rate)? It aims to understand how fluctuations in interest rates influence stock market performance.")
    ...
    st.markdown("It was challenging to deploy streamlit, especially with not understanding the requirements.txt and getting used to its features. I had a difficult time just getting started from scratch on my webapp, so I would've benefited from more knowledge on Streamlit. I also struggled with the API access with my inital project idea, so I needed more skills in this area as this step of the data processing took me the longest time on the project.")
